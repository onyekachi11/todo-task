...
const EditTask = ({
  ...
  return (
    <div className="h-[100%]">
      <div className="bg-[#3556AB] p-10 flex gap-4 text-white font-medium text-[24px] items-center justify-center custom-shadow h-[15%]">
      ...
      <div className="flex flex-col justify-between h-[85%] border ">
        ...
        <div className="flex items-center gap-3 m-5">
          <button
            className="bg-[#AB3535] border border-[#720D0D] px-5 py-3 rounded shadow-lg text-white w-[20%]"
            onClick={() => deleteTask(editTasksValue?.id as number)}
          >
            Delete
          </button>
          <button
            className="bg-[#3556AB] border border-[#0D2972] px-5 py-3 rounded shadow-lg text-white w-[80%] "
            onClick={() => {
              if (option === "new task") {
                addNewTask(taskName);
              } else {
                editTask({ id: editTasksValue?.id, name: taskName });
              }
            }}
          >
            Save
